...
class upload {
        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                if (Tools::substr($file, 0, 4) == 'php:') {
                    ...
                    if (file_put_contents($hash, Tools::file_get_contents('php://input'))) {
                    ...
                    if ($this->uploaded) {
                        ...
                        if (is_array($extension) && sizeof($extension) > 0) {
                            $this->file_src_name_ext      = Tools::strtolower($extension[1]);
                            $this->file_src_name_body     = Tools::substr($this->file_src_name, 0, ((Tools::strlen($this->file_src_name) - Tools::strlen($this->file_src_name_ext)))-1);
                        } else {
                            $this->file_src_name_ext      = '';
                        ...
                        if ($this->mime_fileinfo !== '') {
                            if ($this->mime_fileinfo === true) {
                                if (getenv('MAGIC') === FALSE) {
                                    if (Tools::substr(PHP_OS, 0, 3) == 'WIN') {
    ...
    function temp_dir() {
        ...
        if (!$dir) return '';
        $slash = (Tools::strtolower(Tools::substr(PHP_OS, 0, 3)) === 'win' ? '\\' : '/');
        if (Tools::substr($dir, -1) != $slash) $dir = $dir . $slash;
        return $dir;
    }
    ...
    function getcolors($color) {
        ...
        if (Tools::strlen($color) == 3) $color = str_repeat(Tools::substr($color, 0, 1), 2) . str_repeat(Tools::substr($color, 1, 1), 2) . str_repeat(Tools::substr($color, 2, 1), 2);
    ...
    function getsize($size) {
        $last = Tools::strtolower($size{Tools::strlen($size)-1});
        switch($last) {
            case 'g':
                $size *= 1024;
            case 'm':
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            if (empty($server_path) || is_null($server_path)) {
                ...
                if(Tools::strtolower(Tools::substr(PHP_OS, 0, 3)) === 'win') {
                    if (Tools::substr($server_path, -1, 1) != '\\') $server_path = $server_path . '\\';
                } else {
                    if (Tools::substr($server_path, -1, 1) != '/') $server_path = $server_path . '/';
                }
        ...
        if ($this->processed) {
            ...
            if ($this->no_script) {
                ...
                if ((((Tools::substr($this->file_src_mime, 0, 5) == 'text/' && $this->file_src_mime != 'text/rtf') || strpos($this->file_src_mime, 'javascript') !== false)  && (Tools::substr($file_src_name, -4) != '.txt'))
            ...
            if (!$return_mode) {
                if (!$this->file_auto_rename) {
                    ...
                    if ($file_src_name_ext != $this->file_src_name_ext) {
                        if (Tools::substr($this->file_dst_name_body, -1 - Tools::strlen($this->file_src_name_ext)) == '.' . $this->file_src_name_ext) {
                            $body = Tools::substr($this->file_dst_name_body, 0, Tools::strlen($this->file_dst_name_body) - 1 - Tools::strlen($this->file_src_name_ext));
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                        ...
                        if ($this->image_ratio_x) {
                            ...
                            if ((!$this->image_ratio_no_zoom_in && !$this->image_ratio_no_zoom_out)
                                ...
                                if ($this->image_ratio_crop) {
                                    ...
                                    $this->image_ratio_crop = Tools::strtolower($this->image_ratio_crop);
                                    ...
                                    $this->image_ratio_fill = Tools::strtolower($this->image_ratio_fill);
                                    if (($this->image_src_x/$this->image_x) < ($this->image_src_y/$this->image_y)) {
                                        $this->image_dst_y = $this->image_y;
                                        $this->image_dst_x = intval($this->image_src_x*($this->image_y / $this->image_src_y));
                                        $ratio_crop = array();
                    ...
                    if ($gd_version >= 2 && !empty($this->image_flip)) {
                        $this->image_flip = Tools::strtolower($this->image_flip);
                    ...
                    if ($this->image_watermark!='' && file_exists($this->image_watermark)) {
                        ...
                        $this->image_watermark_position = Tools::strtolower($this->image_watermark_position);
                    ...
                    if (!empty($this->image_text)) {
                        ...
                        $this->image_text_position = Tools::strtolower($this->image_text_position);
                        $this->image_text_direction = Tools::strtolower($this->image_text_direction);
                        $this->image_text_alignment = Tools::strtolower($this->image_text_alignment);

                        // if the font is a string, we assume that we might want to load a font
                        if (!is_numeric($this->image_text_font) && Tools::strlen($this->image_text_font) > 4 && Tools::substr(Tools::strtolower($this->image_text_font), -4) == '.gdf') {
                            ...
                            foreach ($text as $k => $v) {
                                if ($this->image_text_direction == 'v') {
                                    imagestringup($filter,
                                                  ...
                                                  $text_height - (2 * $this->image_text_padding_y) - ($this->image_text_alignment == 'l' ? 0 : (($t_height - Tools::strlen($v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))) ,
                                                  $v,
                                                  $text_color);
                                } else {
                                    imagestring($filter,
                                                $this->image_text_font,
                                                ($this->image_text_alignment == 'l' ? 0 : (($t_width - Tools::strlen($v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))),
                            ...
                            foreach ($text as $k => $v) {
                                if ($this->image_text_direction == 'v') {
                                    imagestringup($image_dst,
                                                  ...
                                                  $text_y + $text_height - (2 * $this->image_text_padding_y) - ($this->image_text_alignment == 'l' ? 0 : (($t_height - Tools::strlen($v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))),
                                                  $v,
                                                  $text_color);
                                } else {
                                    imagestring($image_dst,
                                                $this->image_text_font,
                                                $text_x + ($this->image_text_alignment == 'l' ? 0 : (($t_width - Tools::strlen($v) * $char_width) / ($this->image_text_alignment == 'r' ? 1 : 2))),
                ...
                if (!$return_mode) {
                    ...
                    if (!Tools::copy($this->file_src_pathname, $this->file_dst_pathname)) {
                        $this->processed = false;
                        $this->error = $this->translate('copy_failed');
                    }
                } else {
                    // returns the file, so that its content can be received by the caller
                    $return_content = @Tools::file_get_contents($this->file_src_pathname);
        ...
        while ($Y >= 0) {
            ...
            while ($X < $bmp['width']) {
                if ($bmp['bits_per_pixel'] == 24)
                    $color = unpack("V",Tools::substr($im,$P,3).$vide);
                elseif ($bmp['bits_per_pixel'] == 16) {
                    $color = unpack("n",Tools::substr($im,$P,2));
                    $color[1] = $palette[$color[1]+1];
                } elseif ($bmp['bits_per_pixel'] == 8) {
                    $color = unpack("n",$vide.Tools::substr($im,$P,1));
                    $color[1] = $palette[$color[1]+1];
                } elseif ($bmp['bits_per_pixel'] == 4) {
                    $color = unpack("n",$vide.Tools::substr($im,floor($P),1));
    ...
    function imagebmp(&$im, $filename = "") {
        ...
        for ($y = $h - 1; $y >= 0; --$y) {
            for ($x = 0; $x < $w; ++$x) {
                ...
                $result .=  Tools::substr(pack ('V', $col), 0, 3);
            }
            for ($i = 0; $i < $numpad; ++$i)
                $result .= pack ('C', 0);
        }
